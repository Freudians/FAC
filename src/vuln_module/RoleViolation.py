...
def detector(contract) :
    ...
    We'll call the provided smart contract Zaima. Write a smart contract, in solidity (same version as Zaima) that breaks the access control of Zaima. 
    The smart contract should be called "Exploit", and inherit from the class Test. It should include at least 4 functions:
    1] A constructor, which takes no arguments 
    2] A function called "setup", which deploys an instance of Zaima. The exploit will use this instance of Zaima.
    3] A fallback function to receive payment
    4] A function called "test_exploit", which should break the access control of Zaima. Identify the access control system implemented in the smart contract, and then
    ...
    successful function calls. Feel free to include extra functions that are necessary to achieve your goal, but don't expect them to be called.
    At the top of the program you output, import the library at "forge-std/Test.sol".
